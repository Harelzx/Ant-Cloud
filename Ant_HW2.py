...
"""graph explainer button"""

help_icon = widgets.Button(description='ℹ️ Graph explainer', button_style='info', icon='question')
help_icon.layout = widgets.Layout(align_self='flex-end')
...
help_text = widgets.HTML(
    value=f"<b>Graph Info:</b> {graph_explanation}",
    layout=widgets.Layout(padding='10px', border='1px solid gray', display='none')
)

ft_help_text = widgets.FloatText(    value=7.5,
    description='blah blah graphs',
    disabled=False)


help_display_flag= False

help_window = widgets.VBox([help_icon, help_text])
help_window.layout.align_items = 'flex-end'



def toggle_help_box(b):
    global help_display_flag, graph_explanation
    if help_display_flag:
        help_text.layout.display = 'none'
        help_display_flag = False
    else:
        help_text.layout.display = 'block'
        help_display_flag = True


help_icon.on_click(toggle_help_box)
...
def stats_screen(out):
    ...
    global df_indoor, df_outdoor, graph_explanation

    def plot_by_mode(mode):
      if mode == 'engineer_points':

        graph_explanation = "the amount of points gained by each engineer over time"
        help_text.value = f"<b>Graph Info:</b> {graph_explanation}"
        df = workers_df.copy()
        ...
        graph_explanation = "the efficiency of each station over time"
        help_text.value = f"<b>Graph Info:</b> {graph_explanation}"
        ...
        graph_explanation = "the average number of critical and non-critical errors per station"
        help_text.value = f"<b>Graph Info:</b> {graph_explanation}"
        ...
        graph_explanation = "the points gained per engineer each day"
        help_text.value = f"<b>Graph Info:</b> {graph_explanation}"
        ...
        graph_explanation = "the number of critical and non-critical errors per station each day"
        help_text.value = f"<b>Graph Info:</b> {graph_explanation}"
    ...
    with out:
        ...
        main_container = widgets.VBox([help_window])
        ...
        def update_param_options(*args):
            if sensor_select.value == 'indoor':
                ...
                graph_explanation = "values from the indoor sensor"
                help_text.value = f"<b>Graph Info:</b> {graph_explanation}"
                ...
                graph_explanation = "values from the outdoor sensor"
                help_text.value = f"<b>Graph Info:</b> {graph_explanation}"
